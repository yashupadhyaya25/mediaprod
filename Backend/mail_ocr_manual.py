    ...
    for file in file_list :
                try :
                    ...
                    if ocr_data[0].size > 0 and ocr_data[1].size > 0 :
                        ...
                        #### Check Whether The Invoice Is Already Present In DB Or Not ####
                        db_connection.connect().execute(text("INSERT INTO PDF_AUDIT VALUES ( '"+file+"','COMPLETED','"+"')"))
                        db_connection.dispose()
                        shutil.move(user_upload_pdf_path+file,invoice_folder_path+'completed/'+file)
                    else :
                        db_connection.connect().execute(text("INSERT INTO PDF_AUDIT VALUES ( '"+file+"','COMPLETED','"+"DataFrame Is Empty')"))
                        shutil.move(user_upload_pdf_path+file,invoice_folder_path+'issue/'+file)
                        db_connection.dispose()
                except Exception as e :
                    print(e)
                    db_connection.connect().execute(text("INSERT INTO PDF_AUDIT VALUES ( '"+file+"','ISSUE','"+str(e).replace("'","")+ "')"))
                    shutil.move(user_upload_pdf_path+file,invoice_folder_path+'issue/'+file)
                    db_connection.dispose()
                    continue
else :  
