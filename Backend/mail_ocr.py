    ...
    for file in file_list :
                try :
                    ...
                    if ocr_data[0].size > 0 and ocr_data[1].size > 0 :
                        ...
                        description_detail_df['PER_UNIT_PRICE'] = description_detail_df['PER_UNIT_PRICE'].str.strip().str.replace(' ',',')
                        description_detail_df['TOTAL_AMOUNT'] = description_detail_df['TOTAL_AMOUNT'].str.strip().str.replace(' ',',')
                        ...
                        other_detail_df['TOTAL_TTC'] = other_detail_df['TOTAL_TTC'].astype(str).str.strip().str.replace(' ',',')
                        other_detail_df['TOTAL_TVA'] = other_detail_df['TOTAL_TVA'].astype(str).str.strip().str.replace(' ',',')
                        other_detail_df['TOTAL_HT'] = other_detail_df['TOTAL_HT'].astype(str).str.strip().str.replace(' ',',')
